...
const content = ref(
    props.initialContent || "# Contexto Inicial\nDigite seu prompt aqui...",
);
...
const syncScroll = (source: "visual" | "markdown") => {
    ...
    requestAnimationFrame(() => {
        if (source === "visual") {
            ...
            const percentage =
                vContainer.scrollTop /
                (vContainer.scrollHeight - vContainer.clientHeight);
            // Aplica a mesma porcentagem ao outro container
            mContainer.scrollTop =
                percentage *
                (mContainer.scrollHeight - mContainer.clientHeight);
        } else {
            const percentage =
                mContainer.scrollTop /
                (mContainer.scrollHeight - mContainer.clientHeight);
